...
func main() {
 cases := []struct{Name, Prefix, Value string}{
  ...
  {"permitted_alias", "$sum := $length;", "$string($sum(\"abc\"))"},
  {"permitted_partial_alias", "$sum := $substring(?,1);", "$sum(\"abc\")"},
  {"nested_permitted_alias", "$sum := $length; ($sum := $abs; $sum(-2));", "$string($sum(\"abc\"))"},
  {"ordinary_forbidden_name", "$reverse := 3;", "$string($exists($reverse))"},
 }
 for _, c := range cases {
